'''
_zst: Zack working with .yml
_zst_v2: Modified to work with .yml file that provides parameters of arguments for thresholds
    u_min = min of uniform distribution.
...
def run_trial(
    ...
    **kwargs,
    ...
    if threshold_type == ThresholdType.FIXED:
        ...
        def threshold_fn():
            return np.random.uniform(kwargs["u_min"], kwargs["u_max"])
    elif threshold_type == ThresholdType.NORMAL:

        def threshold_fn():
            # TODO: make these kwargs
            return max(0, np.random.normal(kwargs["n_mu"], kwargs["n_sd"]))
